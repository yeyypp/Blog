public class Main {
     ...
     * 后者可以自由使用外部变量和方法
     *
     * 静态方法，类中只能访问静态变量和静态方法
     *
     * 静态变量： 属于类，类的对象共有，通常和final一起使用作为常量
     * 静态方法： 属于类，可以直接通过类名加方法名调用
     * 静态块： 会在加载类时被创建，且只会创建一次，
     */
    ...
    /**
     * 深拷贝，浅拷贝
     * 一般变量中存储的是bit，基本数据类型中存储的bit就是它的值，引用中存储的bit是对象的地址，
     * 当传递时，实际是拷贝bit，所以当基本数据类型拷贝时，相当于创建了一个新的值，而引用传递时，相当于是传递了一个地址，
     * 使得新的引用还是指向同一个对象
     *
     * 浅拷贝就是在拷贝时，对引用对象，只是把引用传递了过去，没有创建新的对象
     * 深拷贝就是对引用，会创建一个新的对象，并复制原有的内容
     * 需要实现Cloneable并重写public Object clone();
     * @Override
     * public Object clone() {
     *     try {
     *         return super.clone();
 ...
 * 声明变量，静态变量被所有对象共享，在加载类的时候完成静态内存分配，
 * 在jdk1.8以后静态变量存在class对象中，class对象存在堆中
