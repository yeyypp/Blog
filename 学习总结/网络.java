public class Main {
     ...
     * udp是无连接的，不可靠传输，速度快，因为不会建立连接，也没有超时重发机制，所以传输块
     *
     * 为什么tcp是可靠的
     * 通过三次握手，保证连接是可靠的。
     * 通过校验和检查数据是否有误
     ...
     *
     *
     * HTTP协议是无状态的，指的是协议对于事务处理没有记忆能力，服务器不会存储任何关于客户端的信息
     *
     * HTTP1.0 和 1.1 的区别
     * 最主要的区别是 1.1 支持持久连接。Connection 请求头的值为 Keep-Alive 时，客户端通知服务器返回本次请求结果后保持连接；
     * Connection 请求头的值为 close 时，客户端通知服务器返回本次请求结果后关闭连接
     *
     * 长连接省去较多的TCP建立和关闭的操作，但过多的话，服务器承受不了
     *
     *
     * http请求
     ...
     *
     *  请求方法，url， http协议版本
     *  请求方法有 GET, POST OPTION, PUT ,DELETE, HEAD, TRACE, CONNECT
     *  get虽然也能传递数据，但是因为1会在地址栏中显示，2传递数据大小受限，所以一般不用get传递
     *  post会把传递的数据放在请求内容中
     *  head与get类似，但只返回响应头部
     *  put上传数据
     *  delete删除
     ...
     *
     *
     * 一个连接是由源ip 源端口 目标ip 目标端口 协议 确定的
     *
     * tcp time wait
     *
     *
